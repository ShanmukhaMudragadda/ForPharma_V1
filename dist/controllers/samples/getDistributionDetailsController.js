 ...
 */
const getCustomerLocation = (customer, customerType) => {
...
export const getDistributionDetails = async (req, res) => {
    try {
        ...
        const drugs = distribution.drugItems.map((item) => ({
            id: item.id,
        ...
        const gifts = distribution.giftItems.map((item) => ({
            id: item.id,
        ...
        const totalItems = drugs.reduce((sum, item) => sum + item.quantity, 0) +
            gifts.reduce((sum, item) => sum + item.quantity, 0);
