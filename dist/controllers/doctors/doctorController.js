...
export const createDoctorController = async (req, res) => {
    try {
        const { name, designation, specialization, email, phone, description, qualification, experienceYears } = req.body;
...
export const updateDoctor = async (req, res) => {
    try {
        ...
        const { name, designation, specialization, email, phone, description, profilePictureUrl, qualification, experienceYears } = req.body;
        // Validation
        if (!tenantDb) {
            return res.status(500).json({
                success: false,
...
export const getDoctorListController = async (req, res) => {
    try {
        ...
        const doctorList = doctorAssociations
            .filter((assoc) => assoc.doctor.isActive)
            .map((assoc) => ({
...
const groupDoctorsByHospitals = (doctorList) => {
