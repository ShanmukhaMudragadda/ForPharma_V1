...
import { View, Text, SafeAreaView, TouchableOpacity, StatusBar, ScrollView, TextInput, Modal, ActivityIndicator, Alert, RefreshControl } from 'react-native';
...
import RcpaService, { RcpaReport } from '../../services/rcpaService';
...
export default function RCPAPage(): JSX.Element {
  ...
  // Backend data states
  const [rcpaReports, setRcpaReports] = useState<RcpaReport[]>([]);
  ...
  // Load RCPA reports from backend
  const loadRcpaReports = async () => {
    try {
      setError(null);
      const reportsData = await RcpaService.getRcpaList();
      setRcpaReports(reportsData);
    } catch (error: any) {
      console.error('Error loading RCPA reports:', error);
      setError(error.message || 'Failed to load RCPA reports');
      Alert.alert('Error', 'Failed to load RCPA reports. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  // Refresh RCPA reports
  const onRefresh = async () => {
    setRefreshing(true);
    await loadRcpaReports();
    setRefreshing(false);
  };

  // Load RCPA reports on component mount
  useEffect(() => {
    loadRcpaReports();
  }, []);
  ...
  const filteredRCPAData = rcpaReports.filter(rcpa => {
    // Search filter
    ...
    return (
      <StyledSafeAreaView className="flex-1 bg-gray-50">
        ...
        <StyledView className="flex-1 justify-center items-center">
          ...
          <StyledText className="text-gray-600 mt-4">Loading RCPA reports...</StyledText>
        </StyledView>
      </StyledSafeAreaView>
    );
  }
          ...
          filteredRCPAData.map((rcpa) => (
            <RCPACard 
              ...
              data={{
                ...
                totalPrescriptions: rcpa.totalPrescriptions
              }}
              onPress={(rcpaId) => {
