...
const ChemistsView = () => {
  ...
  const chemistChains: { id: string; name: string }[] = [
    { id: 'chain1', name: 'MediCare Chain' },
    { id: 'chain2', name: 'HealthPlus Network' },
    { id: 'chain3', name: 'Pharmacy Express' }
  ];

  const territories: { id: string; name: string }[] = [
    { id: 't1', name: 'Central District' },
    { id: 't2', name: 'East District' },
    { id: 't3', name: 'West District' },
    { id: 't4', name: 'North District' }
  ];

  const [chemistsList] = useState<Chemist[]>(chemists);

  // Form state
  const [chemistForm, setChemistForm] = useState<Record<string, string>>({
    name: '', type: 'independent', chemistChainId: '', territoryId: '', email: '', phone: '',
    address: '', city: '', state: '', pincode: '', latitude: '', longitude: '',
    description: '', profilePictureUrl: '', visitingHours: ''
  });

  const filteredChemists = chemistsList.filter(chemist =>
    chemist.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    chemist.shopName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    getTerritoryName(chemist.territoryId).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  // Removed unused getChemistChainName

  // Mock function to calculate latitude and longitude from address
  const calculateLatLngFromAddress = async (address: string, city: string, state: string, pincode: string) => {
    setIsCalculatingLocation(true);
    try {
      // Mock API call - replace with actual geocoding service
      console.log('Calculating coordinates for:', `${address}, ${city}, ${state} ${pincode}`);
      // const response = await fetch(`https://api.geocoding.com/v1?address=${encodeURIComponent(fullAddress)}`);
      ...
      setTimeout(() => {
        setChemistForm(prev => ({
          ...prev,
          latitude: mockCoordinates.latitude.toFixed(6),
          longitude: mockCoordinates.longitude.toFixed(6)
        }));
  ...
  const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (updatedForm.address && updatedForm.city && updatedForm.state && updatedForm.pincode) {
      calculateLatLngFromAddress(updatedForm.address, updatedForm.city, updatedForm.state, updatedForm.pincode);
    }
  };
  ...
  const handleCreateChemist = async () => {
    ...
    try {
      ...
      // const response = await adminService.createChemist(chemistForm);
      resetForm();
  ...
  const openEditModal = (chemist: Chemist) => {
    ...
    setChemistForm({
      ...
      pincode: chemist.pincode, latitude: chemist.latitude?.toString() || '',
      longitude: chemist.longitude?.toString() || '', description: chemist.description || '',
            ...
            <table className="w-full">
              <thead className="bg-background-tertiary">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider font-heading">Shop Name</th>
              ...
              <tbody className="divide-y divide-border">
                {filteredChemists.map((chemist) => (
                  <tr key={chemist.id} className="hover:bg-background-secondary transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-text-primary">{chemist.shopName}</td>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {chemistForm.type === 'chain' && (
                        <div>
                          ...
                          <select
                            name="chemistChainId"
                            value={chemistForm.chemistChainId}
                            onChange={handleFormChange}
                            className="w-full px-2 py-2 border border-border rounded-sm text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 bg-surface"
                          >
                            <option value="">Select chain</option>
                            {chemistChains.map(chain => (
                  ...
                  <FormCard title="Location Information" subtitle="Address and geographical details">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {isCalculatingLocation && (
                        <div className="md:col-span-2 text-xs text-primary-600 flex items-center">
                          <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-primary-600 mr-2"></div>
                          Calculating coordinates from address...
                        </div>
                      )}
                    </div>
                  </FormCard>
                ...
                <div className="space-y-4">
                  <FormCard title="Basic Information" subtitle="">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div><strong>Shop:</strong> {selectedChemist.shopName}</div>
                  ...
                  <FormCard title="Location" subtitle="">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div><strong>Coordinates:</strong> {selectedChemist.latitude}, {selectedChemist.longitude}</div>
                    </div>
                  </FormCard>
